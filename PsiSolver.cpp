...
double Jtor2( double R, double psi, double psiPrime )
{
	double B = psiPrime/R;
	return -n_i_bar( psi )*omega( psi )*omega( psi )*R/ B;
}
...
int main( int, char** )
{
	...
	std::cout << " Plasma centreline starts at  R = " << R_mid << std::endl;
	...
	for ( unsigned int i=0; i <= N_samples; i++ )
	{
		...
		out << R << '\t' << CoilPsi( R ) << '\t' << psi << '\t' << MidplaneB( R, R_c, Z_c ) << '\t' << MidplaneB( R, R_c, Z_c ) + PsiProblem.Evaluate( B_data, R ) << '\t' << PsiProblem.EvaluateZ( R ) << '\t' << std::endl;
	}
	out.close();
